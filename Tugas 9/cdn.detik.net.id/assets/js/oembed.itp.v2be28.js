...
{var cls='itp_embed';var ifm=$(itbody+" iframe[src*='"+source+"']");if(ifm.length>0)
{var _src=source.replace(/\./g,'');ifm.each(function(i,v){var _cls=cls+'_'+_src+'_'+i;$(v).addClass(_cls);$('.'+_cls).wrap('<div class="sisip_video_ds ratiobox ratiobox--16-9 itp_ratio_counter"></div>');adjustCV.run();});}},getAllAttr:function(arr)
...
$(e).removeClass('video20detik').addClass('video20detik_'+k);$(e).html(`<div class="bumper20detik_${k}" align="center"><img style="${bumper}" src="https://cdn.detik.net.id/assets/images/load.gif"/></div>`);$(window).bind('scroll load',function(){var exist=$('body').find(_ic).length;if(exist==0)
...
{$(e).replaceWith('<div class="sisip_video_ds ratiobox ratiobox--16-9 itp_gambas_video video20detik_'+k+'">'+iframe+'</div>');$('body').find('div.bumper20detik_'+k).remove();window.addEventListener('message',function(event){if(event.data.videois=='play'){$('iframe.video20detik_'+k).css('visibility','visible');}},false);}
else
...
$('body').on('DOMSubtreeModified',itbody+'_wrapper',function(){$.each(embeded,function(i,v){oemhelper.addWrapperClass(v);});});copyArticle();});window.addEventListener('message',function(event){if(event.data.is_next!==undefined){document.getElementById("aevp_title").innerHTML=event.data.is_next;}
if(event.data.login_premium!==undefined){$('.to_login')[0].click();}
var iframe=document.getElementsByClassName("bb-iframe");var height=event.data.height;if(iframe&&iframe[0]){iframe[0].style.height=height+"px";}},false);$(window).bind("load",function(){var frames,b,source,iframe20d,msg,article_id;article_id=$('meta[name=articleid]').attr("content");frames=document.getElementsByTagName("iframe");msg=article_id;for(b=0;b<frames.length;b++){source=frames[b].src;if(source.includes('detik.com')){iframe20d=frames[b].contentWindow;iframe20d.postMessage(msg,'*');}}
